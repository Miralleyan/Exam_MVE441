...
if exercise1c == 1:
    ...
    for run in range(1,11):
        ...
        new_features = pd.concat([pd.DataFrame(data = stats.norm(loc =stats.norm(scale = 4).rvs(), scale = 3).rvs(size = len(x_train)), index=x_train.index , columns=[f"S_{run*add+i}"]) for i in range(add*run)], axis=1)
