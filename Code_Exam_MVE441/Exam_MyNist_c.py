...
from sklearn.cluster import k_means, SpectralClustering, HDBSCAN, AgglomerativeClustering, DBSCAN
...
pd.DataFrame(data = sil[0,:,:]).to_csv("./silhouette_KM")
...
plt.plot(list(range(2,max_cluster+1)), mean_sil_KNN, label = "KMeans")
...
for i, (_, batch_index) in enumerate(skf.split(x_train, y_train)):
    ...
    #label_HD = DBSCAN(eps = 5, min_samples=6).fit_predict(x_batch)
    #print(label_HD)
